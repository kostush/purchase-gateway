...
class SendEmailsCommandHandlerTest extends UnitTestCase
{
    /**
    ...
    protected function setUp(): void
    {
        ...
        $mockRocketGateSettings = $this->createMock(NewCCTransactionInformation::class);
     ...
     *  @return void
     */
    public function send_email_should_contain_transactiondata_cc_details()
    {
        $this->retriveTransactionMock->method('billerName')->willReturn(RocketgateBiller::BILLER_NAME);
        $this->handler->method('retrieveTransactionData')->willReturn($this->retriveTransactionMock);

        $this->assertEquals(true, method_exists($this->retriveTransactionMock->transactionInformation(), 'first6'));
        $this->assertEquals(true, method_exists($this->retriveTransactionMock->transactionInformation(), 'last4'));
        $this->assertEquals(true, method_exists($this->retriveTransactionMock->transactionInformation(), 'cardExpirationMonth'));
        $this->assertEquals(true, method_exists($this->retriveTransactionMock->transactionInformation(), 'cardExpirationYear'));
    }

    /**
     * Legacy at Sec rev with xsell and  existing username  padded both username in main purchase and cross sell
     * and PGW send 2 emails with different username in each email
     *
     * @test
     * @return void
     * @throws \ReflectionException
     */
    public function send_email_should_retrieve_padded_username_from_main_purchase()
    {
        $purchaseProcessed = PurchaseProcessed::createFromJson(json_encode($this->createPurchaseProcessedWithExistingUsernameAndCrossaleWithRocketgateNewPaymentEventData()));
        $site = $this->createSite();
        $this->handler->method('retrieveMemberData')->willReturn( MemberInfo::create(MemberId::create(), Email::create('12@g.co') ));
        $reflection = new \ReflectionClass(SendEmailsCommandHandler::class);
        $method     = $reflection->getMethod('buildMemberInfo');
        $method->setAccessible(true);
        /**
         * @var MemberInfo $memberInfo
         */
        $memberInfo = $method->invoke($this->handler, $site,$purchaseProcessed, []); // handleMainPurchase()
        $this->assertEquals((string) $memberInfo->username(), $purchaseProcessed->subscriptionUsername() );
    }
     ...
     * @throws \ReflectionException
     */
    public function send_email_should_retrieve_padded_username_from_cross_sell_purchase()
    {
        $purchaseProcessed = PurchaseProcessed::createFromJson(json_encode($this->createPurchaseProcessedWithExistingUsernameAndCrossaleWithRocketgateNewPaymentEventData()));
        $crossSalePurchaseData = $purchaseProcessed->crossSalePurchaseData()[0];
        $site = $this->createSite();
        $this->handler->method('retrieveMemberData')->willReturn( MemberInfo::create(MemberId::create(), Email::create('12@g.co') ));
        $reflection = new \ReflectionClass(SendEmailsCommandHandler::class);
        $method     = $reflection->getMethod('buildMemberInfo');
         ...
         * @var MemberInfo $memberInfo
         */
        $memberInfo = $method->invoke($this->handler, $site,$purchaseProcessed, $crossSalePurchaseData); //handleCrossSalePurchases
        $this->assertEquals((string) $memberInfo->username(), $crossSalePurchaseData['subscriptionUsername'] );
    }
}
