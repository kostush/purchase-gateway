...
class RequestWithoutRequiredFieldsTest extends InitPurchase
{
     ...
     * @test
     */
    public function purchase_init_fail_when_subscriptionId_is_provided_and_memberId_is_missing()
    {
        // add subscriptionId to the init payload
        $this->payload['subscriptionId'] = $this->faker->uuid;

        $this->json('POST', $this->validRequestUri(), $this->payload, $this->header());
        $this->assertResponseStatus(Response::HTTP_BAD_REQUEST);

        $response =  json_decode($this->response->getContent(), true);
        $this->assertEquals($response['error'], $this->error_message("member id"));
    }

    /**
     * @test
     */
    public function purchase_init_fail_when_entrySiteId_is_provided_and_memberId_is_missing()
    {
        // add subscriptionId to the init payload
        $this->payload['entrySiteId'] = $this->faker->uuid;
     ...
     * @test
     */
    public function purchase_init_fail_when_memberId_is_provided_but_subscriptionId_or_entrySiteId_is_missing()
    {
        // add subscriptionId to the init payload
        $this->payload['memberId'] = $this->faker->uuid;

        $this->json('POST', $this->validRequestUri(), $this->payload, $this->header());
        $this->assertResponseStatus(Response::HTTP_BAD_REQUEST);

        $response =  json_decode($this->response->getContent(), true);
        $this->assertEquals(Code::INVALID_ENTRY_SITE_SUBSCRIPTION_COMBINATION, $response['code']);
        $this->assertEquals($response['error'], Code::getMessage(Code::INVALID_ENTRY_SITE_SUBSCRIPTION_COMBINATION));
    }
}
